...
const getIpAddressType = (type) => Object.values(ip_addresses_1.IpAddressTypes).find(x => x === type);
const getAuthType = (type) => Object.values(auth_types_1.AuthTypes).find(x => x === type);
...
class Connector {
    ...
    async getOptions({ ipType: rawIpType, authType: rawAuthType, instanceConnectionName, }) {
        const ipType = getIpAddressType(rawIpType);
        if (!ipType) {
            throw new errors_1.CloudSQLConnectorError({
                message: `Invalid IP type: ${String(rawIpType)}, expected PUBLIC or PRIVATE`,
                code: 'EBADCONNIPTYPE',
            });
        }
        const authType = getAuthType(rawAuthType) || auth_types_1.AuthTypes.PASSWORD;
        ...
        return {
            stream() {
                const { instanceInfo, ephemeralCert, host, privateKey, serverCaCert } = instances.getInstance({ instanceConnectionName, ipType, authType });
                if (instanceInfo &&
                    ephemeralCert &&
                    host &&
                    privateKey &&
                    serverCaCert) {
                    return (0, socket_1.getSocket)({
                        instanceInfo,
                        ephemeralCert,
                        host,
                        privateKey,
